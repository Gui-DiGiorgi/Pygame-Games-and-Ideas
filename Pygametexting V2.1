# Now added a way to write multiple lines of text to the screen instead of doing one at a time
# They come in three ways, a initial position, the all the text will initiate from that x and y coordinates
# and move down, a central position, which the center has two modes, first is the middle will be where the chosen x and y are,
# based on how many characters the text has overall. The second mode is where the y coordinate will still be at the middle y in the text,
# but you will control where the x starts if it fits you better than the automatic first method. And to finish, there's a 
# limit position, where the x coordinate text starts is based on the rule that the biggest line of your text cant cross the x limit, 
# and the y will be where the last line ends, the x mode has an aproximmation method which might not work for some text where
# some text displaybox will be larger than others, the calculations considers that, for example, '.' and 'o' take the same space on
# screen, that was chosen to save time calculating the extension of the text for every line given.

class pyg_text():
    
    def __init__(self,Size,Color,Font,Win=None,Color_over=(0,100,200),Color_press=(0,100,100)):
    ...
    def screen_multtext_centerpos(self,text, x_pos, y_pos, y_dist, size=None, color=None, font=None, win=None):
        import pygame
        import sys
        ...
        o_font = pygame.font.SysFont(font, size) 
        ...
        for char,index in zip(text,range(len(text))):
            if index+1 == len(text):
                if char == "\n":
                    self.screen_text_initpos(text[start_point+1:index],x,y-y_sub,size=size,color=color,font=font,win=win)
                else:
                    self.screen_text_initpos(text[start_point+1:index+1],x,y-y_sub,size=size,color=color,font=font,win=win)

            elif char == "\n":
                if lines == 0:
                    self.screen_text_initpos(text[start_point:index],x,y-y_sub,size=size,color=color,font=font,win=win)
                else:
                    self.screen_text_initpos(text[start_point+1:index],x,y-y_sub,size=size,color=color,font=font,win=win)
                start_point = index
                y += y_dist
    ...
    def screen_multtext_initpos(self,text, x_pos, y_pos, y_dist, size=None, color=None, font=None, win=None):
        import pygame
        import sys
        ...
        for char,index in zip(text,range(len(text))):
            if index+1 == len(text):
                if char == "\n":
                    self.screen_text_initpos(text[start_point+1:index],x_pos,y,size=size,color=color,font=font,win=win)
                else:
                    self.screen_text_initpos(text[start_point+1:index+1],x_pos,y,size=size,color=color,font=font,win=win)

            elif char == "\n":
                if lines == 0:
                    self.screen_text_initpos(text[start_point:index],x_pos,y,size=size,color=color,font=font,win=win)
                else:
                    self.screen_text_initpos(text[start_point+1:index],x_pos,y,size=size,color=color,font=font,win=win)
                start_point = index
                y += y_dist
    ...
    def screen_multtext_limitpos(self,text, x_pos, y_pos, y_dist, size=None, color=None, font=None, win=None):
        import pygame
        import sys
        ...
        mn_font = pygame.font.SysFont(font, size) 
    ...
    def screen_button_centerpos(self,text, x_pos, y_pos, trans = None, size=None, color=None, 
        color_over=None, color_press=None, font=None, win=None):
        ...
        if trans == None:
            ...
            self.screen_text_centerpos(text,x_center,y_center, color = color)
            if x_init<pygame.mouse.get_pos()[0]<x_limit and y_init<pygame.mouse.get_pos()[1]<y_limit:
                self.screen_text_centerpos(text,x_center,y_center,color = color_over)
                if pygame.mouse.get_pressed()[0]:
                    self.screen_text_centerpos(text,x_center,y_center,color = color_press)
    ...
    def screen_button_initpos(self,text, x_pos, y_pos, trans = None, size=None, color=None, 
        ...
        if trans == None:
            ...
            self.screen_text_initpos(text,x_init,y_init, color = color)
            if x_init<pygame.mouse.get_pos()[0]<x_limit and y_init<pygame.mouse.get_pos()[1]<y_limit:
                self.screen_text_initpos(text,x_init,y_init,color = color_over)
                if pygame.mouse.get_pressed()[0]:
                    self.screen_text_initpos(text,x_init,y_init,color = color_press)
                    pygame.display.update((x_init,y_init),(total_x,total_y))
    ...
    def screen_button_limitpos(self,text, x_pos, y_pos, trans = None, size=None, color=None, 
        ...
        if trans == None:
            ...
            self.screen_text_limitpos(text,x_limit,y_limit, color = color)
            if x_init<pygame.mouse.get_pos()[0]<x_limit and y_init<pygame.mouse.get_pos()[1]<y_limit:
                self.screen_text_limitpos(text,x_limit,y_limit,color = color_over)
                if pygame.mouse.get_pressed()[0]:
                    self.screen_text_limitpos(text,x_limit,y_limit,color = color_press)
                    pygame.display.update((x_init,y_init),(total_x,total_y))
