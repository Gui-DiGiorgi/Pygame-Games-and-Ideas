# In this version I change some functions (particularly the multi line text with the focus on the middle)
# in order to make it more intuitive for it to work, and I add an option for the bottoms to do something while they
# have the mouse cursor over them, not only when they are clicked. And I remove the trans check to check if the page has been
# transitioned and it can't receive more presses. There are better ways to handle that, like a buttom press check and only
# free it when the mouse buttom is released again (so only one click goes through).
