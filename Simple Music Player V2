# Player V2 = 44.1kHz static sample freq, but allows autoplay/playlist while on background
...
def mp3_freq_clearer(s_freq,file_list):
    import mutagen.mp3
    
    kill=[]
    
    for i in file_list:
        audio_freq = mutagen.mp3.MP3(r'input/{}'.format(i)).info.sample_rate
        if audio_freq!=s_freq:
            kill+=[i]
...
while run:
    ...
    if not start:
        ...
        pygtxt.screen_text_centerpos(start_text,400,150)
